...
func spawn_next_arrow():
	...
	# NEW: Read speed from note, or use default based on difficulty if not specified
	if note.has("speed"):
		speed = float(note.get("speed"))
	else:
...
func _input(event):
	...
	if event is InputEventKey and event.pressed and not event.echo:
			...
			if pressed_arrow in chord_directions:
				...
				if chord_keys_hit.size() == chord_directions.size():
					...
					if offset <= ScoreManager.PERFECT_WINDOW:
						result = "Perfect"
						apply_score_with_multiplier(result, 500)
						for dir in chord_directions:
							_trigger_particles(dir)
							_flash_arrow(dir)
					elif offset <= ScoreManager.GOOD_WINDOW:
						result = "Good"
						apply_score_with_multiplier(result, 200)
					else:
						result = "Miss"
						apply_score_with_multiplier(result, 0)
		...
		if pressed_arrow == target_arrow:
			...
			if offset <= ScoreManager.PERFECT_WINDOW:
				result = "Perfect"
				apply_score_with_multiplier(result, 300)
				_trigger_particles(pressed_arrow)
				_flash_arrow(pressed_arrow)
			elif offset <= ScoreManager.GOOD_WINDOW:
				result = "Good"
				apply_score_with_multiplier(result, 100)
			else:
				result = "Miss"
				apply_score_with_multiplier(result, 0)
