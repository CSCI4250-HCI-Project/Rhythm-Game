...
@onready var song_selection_button = $PausePanel/VBoxContainer/SongSelectionButton
@onready var difficulty_selection_button = $PausePanel/VBoxContainer/DifficultySelectionButton
...
func _ready():
	...
	# Make buttons bigger and connect signals
	if resume_button:
	...
	if song_selection_button:
		song_selection_button.add_theme_font_size_override("font_size", 80)
		song_selection_button.pressed.connect(_on_song_selection_pressed)
	
	if difficulty_selection_button:
		difficulty_selection_button.add_theme_font_size_override("font_size", 80)
		difficulty_selection_button.pressed.connect(_on_difficulty_selection_pressed)
...
func _on_song_selection_pressed():
	is_paused = false
	get_tree().paused = false
...
func _on_difficulty_selection_pressed():
	is_paused = false
	get_tree().paused = false
